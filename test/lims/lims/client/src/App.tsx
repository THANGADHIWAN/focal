...
function App({ productId }: AppProps) {
  ...
  useEffect(() => {
    ...
    if (path.includes('/samples/') && !path.endsWith('/samples')) {
      // We're on a sample details page, don't change activePage
      return;
    }
    if (path.includes('/inventory/') && !path.endsWith('/inventory')) {
      setActivePage('inventory');
      return;
    }
    if (path.includes('/equipment/') && !path.endsWith('/equipment')) {
      setActivePage('equipments');
      return;
    }
    if (path.includes('/tests/') && !path.endsWith('/tests')) {
      setActivePage('tests');
      return;
    }
    if (path.includes('/specifications/') && !path.endsWith('/specifications')) {
      setActivePage('specifications');
      return;
    }
  }, [location.pathname]);
          ...
          <Route 
            ...
            element={
              activePage === 'dashboard' ? (
                ...
                <SampleManagement onSampleClick={(sampleId) => navigate(`/products/${productId}/samples/${sampleId}`)} />
              ) : (
