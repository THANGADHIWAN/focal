...
export default function SampleRegistrationModal({
  ...
  const [availableBoxes, setAvailableBoxes] = useState<StorageLocation[]>([]);
  ...
  // Reset form when modal opens and fetch required metadata
  useEffect(() => {
    if (isOpen) {
      setFormData({
        name: '',
      ...
      const loadAllData = async () => {
        try {
          setLoading(true);
          console.log("Loading metadata and available boxes for sample registration...");

          // Load required metadata when modal opens
          ...
          try {
            ...
            const boxes = await API.metadata.getAvailableBoxes();
            setAvailableBoxes(boxes);
            console.log("Available boxes loaded:", boxes);
          } catch (err) {
            console.error('Failed to fetch available boxes:', err);
          }
        } catch (err) {
          console.error('Error loading data for registration modal:', err);
        } finally {
          setLoading(false);
        }
      };
  ...
  const getSelectedBox = () => {
    ...
    return availableBoxes.find(box => box.id === parseInt(selectedBoxId, 10));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Find selected box location
      const selectedBox = getSelectedBox();
      const location = selectedBox ? selectedBox.name : '';

      // Create new sample via API
      // Convert all ID fields to numbers for API payload
      const sampleData = {
        name: formData.name,
        type: parseInt(formData.type, 10),
        owner: parseInt(formData.owner, 10),
        boxId: formData.boxId ? parseInt(formData.boxId, 10) : undefined,
        location, // Let the server handle the location
      ...
      // Refresh available boxes after registration
      try {
        const boxes = await API.metadata.getAvailableBoxes();
        setAvailableBoxes(boxes);
      } catch (error) {
        console.error("Failed to refresh available boxes:", error);
      }
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  ...
                  {sampleTypes && sampleTypes.length > 0 ? (
                    ...
                    <option disabled>Loading sample types...</option>
                  )}
                </select>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  {users && users.length > 0 ? (
                    users.map(user => (
                      <option key={user.id} value={user.id}>{user.value}</option>
                    ))
                  ) : (
              ...
              <div>
                ...
                >
                  ...
                  {isLoading ? (
                    <option value="" disabled>Loading boxes...</option>
                  ) : availableBoxes && availableBoxes.length > 0 ? (
                    availableBoxes.map(box => (
                      <option key={box.id} value={box.id}>{box.name} (Spaces: {box.availableSpaces})</option>
                    ))
                  ) : (
                    <option disabled>No available boxes</option>
                  )}
                </select>
              </div>
            </div>
              ...
              <h3 className="text-sm font-medium text-gray-700 mb-3">
                ...
                <div>
                  ...
                  <div className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                    {selectedBoxId ? getSelectedBox()?.drawer || '-' : '-'}
                  </div>
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">
                  ...
                  <div className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                    {selectedBoxId ? getSelectedBox()?.rack || '-' : '-'}
                  </div>
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">
                  ...
                  <div className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                    {selectedBoxId ? getSelectedBox()?.shelf || '-' : '-'}
                  </div>
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">
                  ...
                  <div className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                    {selectedBoxId ? getSelectedBox()?.freezer || '-' : '-'}
                  </div>
                </div>
                <div className="col-span-2">
                  <label className="block text-sm text-gray-600 mb-1">
                  ...
                  <div className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                    {selectedBoxId ? getSelectedBox()?.lab || '-' : '-'}
                  </div>
                </div>
              </div>
            </div>
