...
export default function Dashboard({ productId }: DashboardProps) {
  ...
  // Filter data by productId for product-specific dashboard
  const productTests = tests.filter(test => test.productId === productId);
  const productSamples = samples.filter(sample => sample.productId === productId);
  const productSpecifications = specifications.filter(spec => spec.productId === productId);
  // Note: inventory and equipment are typically shared across products, so we keep them as-is
  
  // Calculate metrics for the specific product
  const testsInProgress = productTests.filter(test => test.status === 'In Progress').length;
  const testsCompletedToday = productTests.filter(test => 
    test.status === 'Completed' && 
    new Date(test.createdDate).toDateString() === new Date().toDateString()
  ).length;
  const testsPendingReview = productTests.filter(test => 
    test.status === 'Completed' && 
    test.samples.some(sample => sample.status === 'Completed' && !sample.resultStatus)
  ).length;
  
  const samplesReceivedToday = productSamples.filter(sample => 
    new Date(sample.submissionDate).toDateString() === new Date().toDateString()
  ).length;
  
  const samplesNearingExpiry = productSamples.filter(sample => {
  ...
  const specificationsFailedThisWeek = productSpecifications.filter(spec => {
    // Check if any linked tests have failed results
    const linkedTests = productTests.filter(test => spec.linkedTestIds.includes(test.id));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow">
            ...
            <div className="p-6">
              <KanbanOverview tests={productTests} />
            </div>
          </div>
